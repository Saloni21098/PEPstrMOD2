...
def predict_structure(sequence, out_dir, jobname="test_pred",
    ...
    retries=3, delay=5):
    """
    Predicts protein structure using either ESMFold.
    ...
    if method.lower() == "esmfold":
        ...
        raise ValueError("Invalid method. Choose esmfold")
...
def generate_tleap_input(
    ...
    def write_mods(f, mods):
        """Load prepin/frcmod for all modifications."""
        loaded = set()
        for mod in mods:
            for folder in folder_map.values():
                ...
                if os.path.isfile(frcmod) and frcmod not in loaded:
                    ...
                    loaded.add(frcmod)
        if not loaded and mods:
            print(f"Warning: No .prepin/.frcmod files found for mods: {mods}")

    # --- DRY RUN: get total charge ---
    ...
    if charge is None:
        ...
        raise RuntimeError("Failed to extract charge from TLEaP dry run.")
    ...
    with open(tleap_in, "w") as f:
        ...
        # Automatically detect custom terminal residues (from mod_codes)
        terminal_mods = [m for m in mod_codes if m in os.listdir("fixed_ffnt") or m in os.listdir("fixed_ffct")]
        if terminal_mods:
            f.write("\n# Disable automatic peptide bonding for custom termini\n")
        ...
        # --- N–C cyclization (per-residue logic) ---
        if nc_cycl:
            # Handle sequence type (list or string)
            if isinstance(sequence, list):
                ...
                if len(seq_parts) > 1:
                    first_res = seq_parts[0].upper()
                    last_res = seq_parts[-1].upper()
                    num_res = len(seq_parts)
                else:
                    first_res = sequence[0:3].upper()
                    last_res = sequence[-3:].upper()
                    num_res = len(sequence) // 3

            f.write(f"\n# === N–C Cyclization Setup ===\n")
            f.write(f"# N-terminal residue: {first_res}\n")
            f.write(f"# C-terminal residue: {last_res}\n\n")
...
def main():
    ...
    parser.add_argument(
        "-m", "--method", choices=["esmfold"], default="esmfold"
    )
